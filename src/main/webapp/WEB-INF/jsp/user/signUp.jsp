	...
	$(document).ready(function() {
		...
		$('#loginIdCheckBtn').on('click', function() {
			...
			/* 
				...
				// 아이디 중복확인 완료 됐는지 확인 -> idCheckOk d-none이 없는것, 가지고 있으면 확인하라는 얼럿 띄우기
				...
				// 서버로 보내는 방법
				// 1)submit을 동작시킨다.
				//$(this)[0].submit();//$('#signUpForm')의 [] 첫 번쨰 폼태그 => 화면이 넘어간다.
				
				//2) ajax 호출
				let url = $(this).attr('action'); // 그 태그에 있는 attr(속성을) 가져올 건데 action(=/user/sign_up) 가저온다
				let params =  $(this).serialize//name이 반드시 있어야함 form태그에 있는 name으로 파라미터들 구성
				console.log(params);
				
				//ajax
				$.post(url, params); // 어디에 뭘 보낼건지. (request)
				.done(function(data) { // 성공했을 떄의 처리
					//response
					if (data.code == 1) {
						// 성공 로그인 화면으로 보낸다.
